...
async def test_search_tool_awaitable_inside_running_loop(
    ...
    # The tool routes through the shared fetch core, which queries both engines
    # in parallel; stub DDG too so the call stays offline and returns only the
    # SearXNG results (DDG's auto backend rides primp, which slips past
    # --block-network). Patch at the class, not the module-global instance:
    # monkeypatch would otherwise restore instance attributes onto the shared
    # searxng_client and shadow later class-level stubs.
