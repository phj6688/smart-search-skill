...
- `load_chat_model` (`utils.py`) now loads by `provider/model` string instead of always returning `ChatOpenAI`. It parses the prefix and dispatches through a small provider registry (a plain `{provider: constructor}` dict built on langchain-core plus langchain-openai; no `langchain` meta-package and no `init_chat_model`). A bare name with no `/` (for example `gpt-4o-mini`) still resolves to openai, so the deployed `openai/gpt-4o-mini` ranker default keeps working; `ollama/<model>` loads the local integration. The `temperature` kwarg (HLB-655) is forwarded unchanged, so the evaluator still runs at `temperature=0`. `ChatOpenAI` is now imported lazily inside the openai constructor, the one place the concrete integration is named. No prompt or graph-topology changes.
- `tools.py`: both search paths now share one async fetch core (`_fetch_and_merge`). The agentic `search` tool and `search_direct` each call it; the core runs SearXNG and DuckDuckGo in parallel via `asyncio.gather` and returns the deduplicated merge. The `search` tool no longer falls back to DuckDuckGo sequentially, so the tool path now fetches both engines in parallel like `search_direct`. Deduplication now keys on `normalize_url(link)` rather than the exact link string, so tracking-param and trailing-slash twins collapse. The core makes no LLM call and each request still opens its own `aiohttp.ClientSession`. Behavior note: the tool path now issues one DuckDuckGo request per query even when SearXNG answers; provenance and `METRICS` (outbound_search_requests, engines_used) emit from the shared core on both paths.
- `tools.py`: removed the pre-fetch SearXNG health probe from the `search` tool and deleted the now-unused `SearXNGClient.health_check` method. The probe was a second real `GET /search` per query on top of the actual search. The shared `_fetch_and_merge` core already tolerates an empty or raising SearXNG leg via `return_exceptions`, so DuckDuckGo still merges when SearXNG is down. Behavior note: the SearXNG leg now makes one request per tool query instead of two; an empty SearXNG 200 is a valid zero-result success that contributes nothing to the merge, and the leg's error path fires only when the request raises or times out.

### Removed
- `[project.dependencies]`: dropped `duckduckgo-search` (the legacy package). The shared fetch core already talks to DuckDuckGo through `ddgs`, so `duckduckgo-search` was dead weight that could drift. `ddgs` is now pinned to a tested range with a floor and an upper bound (`ddgs>=9.14,<10`, installed today is 9.14.4). The HLB-662 `RatelimitException` defensive import still tries `ddgs.exceptions` first and falls back to `duckduckgo_search.exceptions`; with the legacy package absent the fallback is simply never exercised, and its held-out fallback test skips when `duckduckgo_search` is not installed. No `src/` code change.
...
### Added
- No-LLM deterministic mode: `run_workflow(query, use_llm=False)` returns the shared fetch core's merged, deduplicated results directly, without entering the agent or evaluator nodes and without constructing any chat model. The success envelope is identical to the LLM path's (`{"status": "ok", "results": [...], "engines_used": [...], "degraded": <bool>, "degraded_reason": ...}`), built from the same `tools.METRICS` attribution record via `_surface_provenance`, so single-engine degradation surfaces as `degraded=True` on both paths and returned URLs stay byte-identical to the fetched ones. A hard fetch failure returns the same typed error envelope the LLM path does. On the LLM path the evaluator payload is now minimized: the model receives an indexed, URL-free, snippet-capped (~200 chars) view of the fetched results and still selects by index, so no raw URL and no oversized snippet reaches the LLM while the returned results keep the fetched bytes verbatim. `tools.METRICS.llm_calls` now increments once per real LLM invocation (agent and evaluator nodes) and stays 0 on the `use_llm=False` path. `.github/workflows/live-canary.yml` gains a default-config leg exercising `run_workflow(query, use_llm=False)` (DuckDuckGo only, no LLM). `tests/test_no_llm.py` pins the deterministic envelope, the degraded flag per engine state, the minimized payload, and URL byte-identity across both paths.
- `[project.optional-dependencies] local`: installs `langchain-ollama` for local-model support (`pip install 'smart-search-skill[local]'`). It is kept out of the base install, so a default deployment stays langchain-core plus langchain-openai; `load_chat_model` imports `langchain_ollama` lazily and only when an `ollama/` model is requested, and raises an `ImportError` naming the `local` extra when it is missing. Importing `search_workflow` without the extra does not fail.
- `tests/test_provider_loading.py`: unit tests for the provider dispatch (bare name and `openai/` both resolve to `ChatOpenAI` targeting `gpt-4o-mini`, the deployed `openai/gpt-4o-mini` default resolves, `temperature` is forwarded, an unknown provider raises `ValueError`, and `ollama/llama3.1` returns `ChatOllama` when the extra is installed or raises a `local`-extra `ImportError` when it is not). `tests/test_provider_egress.py`: drives `run_workflow` end to end with `model="ollama/llama3.1"` under the `egress_guard` fixture, exercising both the agent and evaluator call sites through the local provider and asserting zero OpenAI egress (the OpenAI integration is never constructed and no socket reaches an OpenAI host).
